...
out_time <- seq.POSIXt(as.POSIXct(start), as.POSIXct(stop), by = "6 hour", tz = "UTC")
...
out_cont <- get_output(yaml, model, vars, out_time = out_time)$temp
...
for(i in 1:(length(dates) - 1)) {
  ...
  tmp <- get_output(yaml, model, vars, obs_depths = z_out, out_time = out_time)$temp
  unlink(file.path(model, "output"), recursive = TRUE)
  out_l[[length(out_l) + 1]] <- tmp
}
...
wtr.lineseries(wtemp_sim_restart)
